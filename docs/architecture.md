...
Structured methods with exactly one final tool and no runtime tools expose only that final tool and use `tool_choice: "required"`. With a single available tool this still constrains the model to the intended final function while avoiding provider-specific failures from named-function forcing. Cortex continuation vs finalization is represented by one `final_cortex_step` tool with a `decision` field, avoiding multi-final-tool control. Methods with real runtime tools use `auto` so the model can call those tools before the final tool.
